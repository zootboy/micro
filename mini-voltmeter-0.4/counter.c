...
int main(void)
{
	...
	unsigned char debounce=0;
        // enable PB1, as digital input
        DDRB&= ~(1<<DDB1);
        PORTB|= (1<<PINB1); // internal pullup resistor on, jumper goes to ground
        DDRB&= ~(1<<DDB2);
        PORTB|= (1<<PINB2); // internal pullup resistor on, jumper goes to ground
	init7segment();
		...
		if (debounce==0){
			if (bit_is_clear(PINB,PINB2)){
				lc=0;
				debounce=15; // wait polling the button to debounce
			}
			if (bit_is_clear(PINB,PINB1)){
				lc++;
				debounce=20; // wait polling the button to debounce
			}
		}else{
			debounce--;
		}
