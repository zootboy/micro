...
// Macro shortenings.
#define SPI_WRITE(data) SPDR = (data);while(!(SPSR & (1<<SPIF))) delay_us(5); delay_us(10)
...
#define NRF_SPCR (SPI_INTERRUPT<<SPIE)|(1<<SPE)|(0<<DORD)|(1<<MSTR)|(1<<SPR1) // Interrupt, enable, LSB 1st, master mode, rising=sample, Fosc/4.
