...
int ffind (const char *pattern, FINFO *info) {
  ...
  for (;;) {
    if (fcb->drive == 4) {
      if (fat_ffind (pattern, info, fcb) == 0) {
        rval = (1); goto end_of_func;;
      }
    }
    else {
      if (_ffind (info, fcb) == 0) {
        rval = (1); goto end_of_func;;
      }
    }
    info->name[255] = 0;
