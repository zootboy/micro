...
int _fcreate (const char *fname, IOB *fcb) {
  ...
  for (i = 0; i < fcb->NumSect; i++) {
    ...
    if ((fa.end ^ 0x0F) == fcb->InitVal) {
       
      if (fs_check_fwopen (i, fcb) == 1) {
        continue;
      } 
       
      if (fs_get_freeMem (i, fcb) == 0) {
        fa.end = fcb->InitVal ^ 0xFF;
