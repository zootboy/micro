...
#line 13 "FlashFS\\fs_mmc.c"

 
#line 28 "FlashFS\\fs_mmc.c"
...
BOOL mmc_read_sect (U32 sect, U8 *buf, U32 cnt) {
  ...
  if (cnt > 1) {
    ...
    retv = mmc_read_block ((0x40 + 18), sect * 512, buf, cnt);

    mmc_command ((0x40 + 12), 0);
    ...
    retv = mmc_read_block ((0x40 + 17), sect * 512, buf, 1);
  }
x:spi_ss (1);
  return (retv);
}
...
BOOL mmc_write_sect (U32 sect, U8 *buf, U32 cnt) {
  ...
  if (cnt > 1) {
    ...
    retv = mmc_write_block ((0x40 + 25), sect * 512, buf, cnt);
    ...
    retv = mmc_write_block ((0x40 + 24), sect * 512, buf, 1);
  }
x:spi_ss (1);
  return (retv);
}
...
BOOL mmc_read_config (MMCFG *cfg) {
  ...
  spi_ss (0);
  retv = mmc_read_bytes ((0x40 + 10), 0, buf, 16);
  spi_ss (1);
  if (retv == 0) {
  ...
  spi_ss (0);
  retv = mmc_read_bytes ((0x40 + 9), 0, buf, 16);
  spi_ss (1);
  if (retv == 0) {
