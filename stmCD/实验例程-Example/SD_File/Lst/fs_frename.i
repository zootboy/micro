...
int frename (const char *oldname, const char *newname) {
  ...
  if (fcb->drive == 4) {
    ...
    if (fat_find_file (newname_w_path, fcb) == 1) {
       
      rval = (1); goto end_of_func;;
    }
    if (fat_rename (oldname, newname, fcb) == 1) {
  ...
  if (fs_Find_File (oldname, fcb) == 0) {
    rval = (_frename (newname, fcb)); goto end_of_func;;
  }
