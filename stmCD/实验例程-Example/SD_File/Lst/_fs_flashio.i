...
BOOL fs_check_fwopen (U32 block, IOB *fcb) {
  ...
  for (i = 0; i < _NFILE; i++) {
    ...
    if ((_iob[i].flags & 0x0002) && (_iob[i]._fblock == block)) {
...
BOOL fs_AllocBlock (IOB *fcb) {
  ...
  for (i = 0; i < fcb->NumSect; i++) {
    ...
    if ((bflag ^ 0x0F) == fcb->InitVal) {
       
      if (fs_check_fwopen (bl, fcb) == 0) {
...
U32 fs_rd_alloc (U32 adr, FALLOC *fa) {
  ...
  if (adr & 0x01) {
    ...
    pa = (U32 *)fa;
    pa[0] = (*((U32 volatile *) (adr)));
    pa[1] = (*((U32 volatile *) (adr+4)));
  }
  return (adr - 8);
}
