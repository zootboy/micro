...
int __fopen (const char *fname, int openmode) {
  ...
  fcb->flags = (openmode & (4 | 8)) ? 0x0002 : 0x0001;
  if (openmode & 8) {
    fcb->flags |= 0x0004;
  }
  ...
  if (fcb->drive == 4) {
    ...
    fid = (fat_find_file (fname, fcb) == 1) ? 0 : 1;
  }
  else {
  ...
  if (fid == 0) {
    ...
    for (i = 0; i < _NFILE; i++) {
      ...
      if ((_iob[i].flags & 0x0002) || (fcb->flags & 0x0002)) {
    ...
    if (fcb->flags & 0x0004) {
      ...
      fcb->fsize  = __getfsize (fcb, 1);
      rval = (handle); goto end_of_func;;
    }
