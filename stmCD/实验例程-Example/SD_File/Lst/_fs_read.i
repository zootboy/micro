...
int __read (int handle, U8 *buf, U32 len) {
  ...
  if (fcb->drive == 4) {
    ...
    if (rlen == 0) {
      ...
      rval = (0x80000000 | len); goto end_of_func;;
    }
     
    rval = (len - rlen); goto end_of_func;;
  }

  for (nr = 0; nr < len;  ) {
    if (fcb->_ftop == fcb->_fbot) {
      if (fs_mark_fileMem (fcb) == 0) {
         
        if (nr == 0) {
           
          rval = (0x80000000 | len); goto end_of_func;;
        }
        rval = (len - nr); goto end_of_func;;
      }
    }
    rlen = len - nr;
