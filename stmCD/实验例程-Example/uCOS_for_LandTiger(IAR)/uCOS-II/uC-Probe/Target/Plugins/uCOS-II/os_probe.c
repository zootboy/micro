...
*                   a prescaler in order to decrease the probability of timer overflows.
*
...
*                                  128 * 65536
*
*                   A possible solution is to call this function from the tick handler of the application's
*                   OS (assuming the tick rate is greater than 5.72 Hz), discarding the return value.
*********************************************************************************************************
*/
...
static  void  OSProbe_Task (void *p_arg)
{
            ...
            INT16U   i;
            INT32U   cycles_tot;
    static  INT32U   cycles_dif[OS_PROBE_N_TASKS];
    static  INT32U   cycles_tot_last[OS_PROBE_N_TASKS];
            INT32U   max;
    ...
    while (1) {
        ...
        while ((i     <  OS_PROBE_N_TASKS) &&
            ...
            if (ptcb->OSTCBStkSize == 0) {
                ...
                if (ptcb->OSTCBStkBase == (OS_STK *)0) {
                    OSProbe_TaskStkUsage[i] = (FP32)0;

                } else {
                    max = (ptcb->OSTCBStkSize) * sizeof (OS_STK);

                    #if (OS_STK_GROWTH == 1)
                    OSProbe_TaskStkUsage[i] = (FP32)(((INT32U)(ptcb->OSTCBStkBase) - (INT32U)(ptcb->OSTCBStkPtr))  * 100) / max;
                    #else
                    OSProbe_TaskStkUsage[i] = (FP32)(((INT32U)(ptcb->OSTCBStkPtr)  - (INT32U)(ptcb->OSTCBStkBase)) * 100) / max;
                    #endif
                }
#else

                if (ptcb->OSTCBStkBase == (OS_STK *)0) {
                    OSProbe_TaskStkUsage[i] = (INT16U)0;
...
#if (OS_PROBE_USE_FP > 0)
        if (cycles_tot == 0) {
            for (i = 0; i < OS_PROBE_N_TASKS; i++) {
                OSProbe_TaskCPUUsage[i] = (FP32)0;
            }
        } else {
            for (i = 0; i < OS_PROBE_N_TASKS; i++) {
        ...
        if (cycles_tot == 0) {
            for (i = 0; i < OS_PROBE_N_TASKS; i++) {
                OSProbe_TaskCPUUsage[i] = (INT16U)0;
            }
